...
class Board:
    ...
    def get_possible_moves(self, color):
        ...
        return moves
    def get_unicode_for_piece(self, piece):
        if piece.piece_type == pieces.King.PIECE_TYPE:
            return '♔' if piece.color == pieces.Piece.WHITE else '♚'
        ...
        if piece.piece_type == pieces.Pawn.PIECE_TYPE:
            return '♙' if piece.color == pieces.Piece.WHITE else '♟'
        return "?"  # Just in case there's an unknown piece type
    ...
    def to_string(self):
        ...
        for y in range(Board.HEIGHT):
            ...
            for x in range(Board.WIDTH):
                piece = self.chesspieces[x][y]
                if piece != 0:
                    string += self.get_unicode_for_piece(piece) + "  "
                else:
                    string += ".  "
            string += "\n"
        return string + "\n"